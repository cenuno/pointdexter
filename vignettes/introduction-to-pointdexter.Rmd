...
`pointdexter` is compatible with the two packages most useful for working with spatial data: [`sf`](https://r-spatial.github.io/sf/) and [`rgdal`](https://www.rdocumentation.org/packages/rgdal/versions/1.3-6).

*See [here](https://cran.r-project.org/doc/contrib/intro-spatial-rl.pdf) for an introduction into visualizing spatial data in R using the `rgdal` package. Otherwise, see [this tutorial](https://r-spatial.github.io/sf/articles/sf5.html) for the `sf` package.*

```{r load necessary packages, message = FALSE}
# load necessary packages -----
library(pointdexter) # label coordinate pairs in polygons
library(sf)          # analyze simple features
library(rgdal)       # analyze SpatialPolygonsDataFrames
...
`pointdexter` makes both polygons available as `sf` and [`SpatialPolygonsDataFrame`](https://www.rdocumentation.org/packages/sp/versions/1.2-5/topics/SpatialPolygonsDataFrame-class) (spdf) objects. This vignette will use both types of objects interchangeably.
...
## Obtain boundaries of the polygon(s) with `GetPolygonBoundaries()`
...
kable(lapply(community.area.boundaries[c("AUSTIN", "WEST ELSDON")], head)
      , caption = "Table 3. Austin (left) and West Elsdon's (right) boundaries"
      , col.names = c("long", "lat"))
```

## `LabelPointsWithinPolygons()` identifies which longitudinal and latitudinal points lie within the polygon(s)
...
kable(head(cps_sy1819[, c(relevant.columns, "in_chicago", "community")])
      , caption = "Table 4. A character vector is returned when polygon.boundaries is a list of labeled matrices")
```
...
# show results
kable(austin.hs[, c(relevant.columns
                    , "overall_rating", "is_high_school", "community")]
      , caption = "Austin's highest rank high school is YCCS - Scholastic Academy, SY1819"
      , row.names = FALSE)
