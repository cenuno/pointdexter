...
library(kableExtra)  # general purpose package for dynamic report generation
```
...
cps_sy1819[, relevant.columns] %>%
  head() %>%
  kable(caption = "Table 1. Examining CPS SY1819 school profile data") %>%
...
boundary %>%
  head() %>%
  kable(caption = "Table 2. boundary is a matrix of coordinate pairs"
...
community.area.boundaries[c("AUSTIN", "WEST ELSDON")] %>%
  lapply(FUN = head) %>%
  kable(caption = "Table 3. Austin (left) and West Elsdon's (right) boundaries"
        , col.names = c("long", "lat")) %>%
  kable_styling(bootstrap_options = c("striped", "hover"))
```
...
cps_sy1819[, c(relevant.columns, "in_chicago")] %>%
  head() %>%
  kable(caption = "Table 4. A logical vector is returned when polygon.boundaries is a matrix") %>%
  kable_styling(bootstrap_options = c("striped", "hover"))
```
...
# show first few records ----
cps_sy1819[, c(relevant.columns, "in_chicago", "community")] %>%
  head() %>%
  kable(caption = "Table 5. A character vector is returned when polygon.boundaries is a list of labeled matrices") %>%
  kable_styling(bootstrap_options = c("striped", "hover"))
```
...
# show results
austin.hs[, c(relevant.columns , "overall_rating",
              "is_high_school", "community")] %>%
  kable(caption = "Table 6. Austin's highest rank high school is YCCS - Scholastic Academy, SY1819"
        , row.names = FALSE) %>%
  kable_styling(bootstrap_options = c("striped", "hover"))
```
